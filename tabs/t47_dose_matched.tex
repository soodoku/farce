\begin{table}
\caption{Holding measured dose fixed. Every album on the US daily chart whose first-day streams reach the smallest of the paper's ten is a comparison unit, which makes the comparison set a rule rather than a judgement about which releases were major. Effects are means of donut residuals with standard errors across albums.}
\begin{tabular}{lrrrrrr}
\toprule
group & n & effect & se & ci\_lower & ci\_upper & p\_value \\
\midrule
the paper's ten & 10 & 16.10 & 5.30 & 4.12 & 28.09 & 0.01 \\
every other album reaching 32.1M & 17 & -0.49 & 2.54 & -5.87 & 4.89 & 0.85 \\
the same, 2018-2022 only & 9 & -1.24 & 2.04 & -5.94 & 3.46 & 0.56 \\
albums below 32.1M & 497 & 0.73 & 0.68 & -0.61 & 2.07 & 0.29 \\
\bottomrule
\end{tabular}
\end{table}
