...
def compute_dynamic_effects(df_global, window=5):
    ...
    print(f"{'Day':>5} | {'Effect':>10} | {'SE':>8} | {'95% CI (t)':>20} | {'n':>3}")
    print("-" * 55)
...
def time_of_day_analysis(accidents, df_global, window=10):
    ...
    print(
        f"{'Time Window':<12} | {'Hours':>20} | {'Baseline':>10} "
        f"| {'Effect':>10} | {'SE':>8} | {'% Total':>8}"
    )
    print("-" * 80)
    for _, r in results_df.iterrows():
        print(
...
def _fisher_ci(r, n, alpha=0.05):
    """Fisher z interval for a Pearson correlation."""
    if n < 4 or abs(r) >= 1:
        return np.nan, np.nan
    z = np.arctanh(r)
    half = stats.norm.ppf(1 - alpha / 2) / sqrt(n - 3)
...
def stream_effect_correlation(df, window=10):
    ...
    r_lo, r_hi = _fisher_ci(r_pearson, len(streams))
    ...
    # What this test could detect if a relationship were there. At these
    # sample sizes only a very large correlation clears significance, so a
    # null carries almost no information.
    n_obs = len(streams)
    t_crit = stats.t.ppf(0.975, n_obs - 2)
    r_min = t_crit / np.sqrt(t_crit**2 + n_obs - 2)
    r_80 = np.tanh(
        (stats.norm.ppf(0.975) + stats.norm.ppf(0.8)) / np.sqrt(max(n_obs - 3, 1))
    )
    print(f"\n  DETECTABLE AT n = {n_obs}: |r| must exceed {r_min:.2f} to reach")
    print(f"  p < 0.05, and {r_80:.2f} for 80% power. An observed |r| below that")
    print("  says nothing about whether a relationship exists.")

    if p_pearson >= 0.05:
        print(f"\n  The correlation is not distinguishable from zero (n = {n_obs}).")
        print("  The interval spans everything from a strong negative relationship")
        print("  to a moderate positive one, so this is an absence of evidence")
        print("  about dose-response, not evidence of its absence.")
        print("\n  Three reasons to expect a null here even if the mechanism is")
        print("  real. Eight of the twenty streaming counts are estimated from")
        print("  chart position rather than measured, and noise in x attenuates a")
        print("  true relationship toward zero. The albums are all large, so the")
        print("  range of the dose is narrow. And first-day streams is a level")
        print("  when the mechanism needs an increment over the artist's normal")
        print("  daily streaming, which is not in the data: 184.7M for a Taylor")
        print("  Swift release may be a smaller jump than 55M for a less")
        print("  established artist.")
    elif r_pearson < 0:
    ...
    if abs(t_stat) < crit:
        print("\n  β₁ is not distinguishable from zero. The interval is wide")
        print("  enough to contain a substantial dose-response, so this test")
        print("  does not discriminate between the hypotheses.")
    elif beta_reg[1] < 0:
    ...
    return {
        ...
        "r_pearson": r_pearson,
        "r_pearson_p": p_pearson,
        "r_pearson_ci_lower": r_lo,
        "r_pearson_ci_upper": r_hi,
...
def dose_response_analysis(df, window=10):
    ...
    st_t1 = album_stats(deltas_t1)
    avg_t1, se_t1 = st_t1["effect"], st_t1["se"]
    print(
    ...
    st_t2 = album_stats(deltas_t2)
    avg_t2, se_t2 = st_t2["effect"], st_t2["se"]
    print(
...
def alcohol_decomposition(accidents, miacc, window=10, bac_threshold=8):
    """
    Where does the release-day excess land, alcohol-involved crashes or not?

    THIS IS A DECOMPOSITION, NOT A MECHANISM TEST. Whether the extra crashes
    involve a drinking driver is itself an outcome, so splitting on it
    conditions on a post-treatment characteristic. It can say where the excess
    sits. It cannot say why, and a larger effect among sober crashes is
    consistent with any mechanism that concentrates ordinary daytime exposure,
    not with distraction specifically.

    An earlier version used the accident file's DRUNK_DR column, which NHTSA
    dropped after 2020, leaving two of the ten albums and no usable standard
    error. NHTSA's multiply-imputed BAC files ship in every annual archive, so
    the restriction was a property of the file being read. Using them restores
    all ten.

    Imputation is handled properly rather than by averaging first. For each of
    the ten imputations the crash is classified as alcohol-involved when the
    imputed BAC reaches `bac_threshold` (units of 0.01 g/dL, so 8 is the 0.08
    legal limit), the daily series is split, the estimator is run, and the ten
    resulting estimates are combined by Rubin's rules. Averaging the
    imputations into a single probability first would understate the
    uncertainty by discarding the between-imputation variance.

    Returns DataFrame with one row per split plus the per-imputation detail.
    """
    print(f"\n{'='*70}")
    print("ALCOHOL DECOMPOSITION (NOT A MECHANISM TEST)")
    print(f"{'='*70}")
    print("Where the release-day excess sits, by whether a driver was at or")
    print(f"above BAC 0.{bac_threshold:02d}. The split is post-treatment: it")
    print("describes where the effect lands, it does not identify why.\n")

    acc = accidents.copy()
    acc.columns = [c.upper() for c in acc.columns]
    imp = miacc.copy()
    imp.columns = [c.upper() for c in imp.columns]

    key = ["YEAR", "ST_CASE"]
    merged = acc.merge(imp, on=key, how="inner", validate="one_to_one")
    if len(merged) != len(acc):
        raise SystemExit(
            f"MIACC join lost rows: {len(acc)} crashes in, {len(merged)} out. "
            "The join must be one-to-one within a year."
        )
    print(f"  joined {len(merged):,} crashes to their imputed BAC values")

    merged = merged.dropna(subset=["YEAR", "MONTH", "DAY"])
    merged["_date"] = pd.to_datetime(
    ...
    shares = []
    per_imputation = {"alcohol": [], "no_alcohol": []}

    for m in range(1, 11):
        col = f"A{m}"
        involved = pd.to_numeric(merged[col], errors="coerce") >= bac_threshold
        shares.append(float(involved.mean()))

        for label, mask in [("alcohol", involved), ("no_alcohol", ~involved)]:
            daily = (
                merged.loc[mask]
                .groupby("_date")["FATALS"]
                .sum()
                .reindex(pd.date_range(merged["_date"].min(), merged["_date"].max()))
                .fillna(0)
                .rename("fatalities")
                .reset_index()
            ...
            daily = add_time_features(daily)
            est_mask = (~daily["date"].dt.date.isin(exclude)).values
            X = build_design_matrix(daily)
            y = daily["fatalities"].values.astype(float)
            beta, _, _ = ols_fit(X.values[est_mask], y[est_mask])
            resid = y - X.values @ beta
            effects = [
                resid[(daily["date"] == d).values][0]
                for d in release_dates
                if (daily["date"] == d).any()
            ]
            st = album_stats(effects)
            per_imputation[label].append((st["effect"], st["se"] ** 2, st["n"]))

    mean_share = float(np.mean(shares))
    print(
        f"  share of crashes at or above BAC 0.{bac_threshold:02d}: "
        f"{mean_share:.3f} (across imputations "
        f"{min(shares):.3f} to {max(shares):.3f})"
    )

    rows = []
    for label, pretty in [
        ("no_alcohol", f"No driver at BAC 0.{bac_threshold:02d}+"),
        ("alcohol", f"Driver at BAC 0.{bac_threshold:02d}+"),
    ]:
        ests = [e for e, _, _ in per_imputation[label]]
        vars_ = [v for _, v, _ in per_imputation[label]]
        n_alb = per_imputation[label][0][2]
        pooled = rubin_combine(ests, vars_)
        rows.append(
            {
                "sample": pretty,
                "n_albums": n_alb,
                ...
                "mean_within_se": float(np.sqrt(np.mean(vars_))),
                "frac_missing_info": pooled["frac_missing_info"],
            }
        )
    ...
    for _, r in results_df.iterrows():
        ...
        print(
            f"{r['sample']:<28} | {int(r['n_albums']):>3} | {r['effect']:>+8.2f} | "
            f"{r['se']:>6.2f} | {ci:>18} | {r['p_value']:>6.3f}"
        )

    total = results_df["effect"].sum()
    print("\nINTERPRETATION:")
    print(f"  The two splits sum to {total:+.1f} deaths, against the all-crash")
    print("  release-day effect. Both are decompositions of the same excess.")
    print("  A larger effect among crashes without a drinking driver is")
    print("  consistent with distraction and with any other mechanism that")
    print("  concentrates ordinary exposure. It does not distinguish them.")
...
def covid_sensitivity(df_global, window=10):
    ...
    if covid_result and non_covid_result:
        ...
        if covid_eff > non_covid_eff * 1.5:
            print(
                f"  WARNING: COVID albums effect ({covid_eff:+.1f}) "
                f">> non-COVID ({non_covid_eff:+.1f})"
            )
...
def extended_series_analysis(df_global):
    ...
    tiers = [
        ...
        # The paper's own ten are inside "All combined", so that row cannot
        # serve as evidence from outside the sample being evaluated. This one
        # can.
        (
            "Excluding the paper's 10",
            ALBUMS_TIER0 + ALBUMS_TIER2 + ALBUMS_TIER3,
        ),
    ]
    ...
    for tier_name, album_list in tiers:
        ...
        st = album_stats(resids)
        n_positive = int(np.sum(np.array(resids) > 0))
        sign_p = stats.binomtest(n_positive, len(resids), 0.5).pvalue
