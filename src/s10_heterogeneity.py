...
def stream_effect_correlation(df, window=10):
    ...
    if p_pearson >= 0.05:
        print(
            f"\n  The correlation is not distinguishable from zero (n = {len(streams)})."
        )
        ...
        print("  about dose-response, not evidence of its absence. Note also that")
        print("  the Tier 2 streaming counts are estimates from chart position,")
        print("  so part of the x-axis is constructed rather than measured.")
    elif r_pearson < 0:
...
def drunk_vs_sober_analysis(accidents, window=10):
    ...
    **This leaves two Tier 1 albums: Scorpion (2018) and Folklore (2020).**
    Six of the ten releases are from 2021 or later. A contrast between two
    treated units carries no usable standard error — with n = 2 the standard
    error is |x1 - x2| / (2 * sqrt(2)) and the implied t-statistic is
    arithmetic, not evidence. album_stats() therefore returns NaN for the SE
    and t-statistic here, and the result should be read as a description of two
    dates. Any write-up of this comparison must state n = 2.

    Terminology: FARS DRUNK_DR counts drivers at BAC >= 0.08, so the "sober"
    split is "no driver at or above the legal limit", not "no driver had been
    drinking". Drivers at BAC 0.01-0.07 fall on the sober side.
    ...
    for label, mask in [
        ("No driver at BAC>=0.08", ~df["_drunk"]),
        ("Driver at BAC>=0.08", df["_drunk"]),
    ]:
        subset = df[mask].copy()
            ...
            results.append(
                {
                    ...
                    "se": st["se"],
                    "t_stat": st["t_stat"],
                    "pct_effect": 100 * st["effect"] / baseline,
                    "n_albums": st["n"],
                }
            )
    ...
    n_units = int(results_df["n_albums"].max())
    print("\nMECHANISM TEST RESULT:")
    if n_units < MIN_ALBUMS_FOR_SE:
        print(f"  Based on {n_units} album(s) only — DRUNK_DR is absent from FARS")
        print("  after 2020, so six of the ten Tier 1 releases drop out. No")
        print("  standard error is reported and none should be inferred. Treat")
        print("  the numbers below as a description of those dates.")
    sober = results_df[results_df["sample"].str.startswith("No driver")]
    drunk = results_df[results_df["sample"].str.startswith("Driver at")]
