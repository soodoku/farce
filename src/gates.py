...
def check_miacc_join(accidents, miacc, bac_threshold=8):
    """
    The imputed-BAC file must cover every crash, exactly once.

    ST_CASE repeats across years, so the join key is (YEAR, ST_CASE). A silent
    many-to-one here would duplicate fatalities into the alcohol split; a
    silent loss would drop them. Both would look like a finding.

    The share of crashes at or above the legal limit is also checked against a
    plausible band. NHTSA puts alcohol-impaired involvement near a third of
    fatal crashes, so a value far outside that means the units are wrong: these
    columns are in 0.01 g/dL, and reading them as g/dL or as parts per
    thousand would move the share to zero or to one without any other symptom.
    """
    acc = accidents.copy()
    ...
    key = ["YEAR", "ST_CASE"]
    if imp.duplicated(key).any():
        raise AssertionError("MIACC has duplicate (YEAR, ST_CASE) rows")

    merged = acc[key].merge(imp[key], on=key, how="inner")
    if len(merged) != len(acc):
        raise AssertionError(
            f"MIACC join is not one-to-one: {len(acc)} crashes, {len(merged)} matched"
        )

    share = float((pd.to_numeric(imp["A1"], errors="coerce") >= bac_threshold).mean())
    if not 0.25 <= share <= 0.40:
        raise AssertionError(
            f"share of crashes at BAC >= 0.{bac_threshold:02d} is {share:.3f}, "
            "outside the plausible band. Check the units on A1-A10."
        )

    return len(merged), share


def run_all(daily, window=10, accidents=None, miacc=None):
    ...
    if accidents is not None and miacc is not None:
        n_joined, share = check_miacc_join(accidents, miacc)
        print(
            f"  imputed BAC: {n_joined:,} crashes joined one-to-one, "
            f"{share:.3f} at or above 0.08"
        )
