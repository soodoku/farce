...
def plot_dose_response(
    ...
    summary,
    show=True,
    path="figs/dose_response.png",
    paper_keys=None,
    n_bins=20,
):
    """
    Release-day excess against measured first-day streams, every charting album.

    Design notes, since two earlier versions of this figure misled.

    The dose spans two orders of magnitude, 0.6 to 80 million, so a linear axis
    puts nine albums in ten inside the left tenth of the panel and makes the
    fit look like it is driven by a handful of points on the right. The axis is
    logarithmic.

    Bins are equal-count and are not joined by a line. Connecting them invites
    the eye to read a trend between the last two, which in the decile version
    was a straight segment drawn through empty space between 14 and 32 million.
    Points that are independent estimates are drawn as independent estimates.

    The ten albums in the original paper are marked, because where they sit in
    the dose distribution is itself the finding.
    """
    import matplotlib.pyplot as plt

    fd = per_album.dropna(subset=["effect", "first_day_millions"]).copy()
    ...
    mark = fd.iloc[:0]
    if paper_keys:
        _keys = {(a, pd.Timestamp(d)) for a, d, _ in paper_keys}
        is_paper = np.array(
            [
                (a, pd.Timestamp(d)) in _keys
                for a, d in zip(fd["lead_artist"], fd["date"])
            ]
        )
        mark = fd[is_paper]
        cut = mark["first_day_millions"].min()
        matched = fd[(~is_paper) & (fd["first_day_millions"] >= cut - 1e-9)]
        rest = fd[(~is_paper) & (fd["first_day_millions"] < cut - 1e-9)]
    else:
        rest, matched, cut = fd, fd.iloc[:0], None

    ax1.scatter(
        rest["first_day_millions"],
        rest["effect"],
        s=13,
        alpha=0.28,
        color="#5b7fa6",
        edgecolor="none",
        zorder=2,
        label=f"one album (n = {summary['n']})",
    )
    if cut is not None:
        for ax in (ax1, ax2):
            ax.axvline(cut, color="0.72", lw=1, ls=(0, (4, 3)), zorder=1)
        ax1.scatter(
            matched["first_day_millions"],
            matched["effect"],
            s=34,
            color="#2f6f4e",
            edgecolor="none",
            alpha=0.85,
            zorder=3,
            label=f"same dose, not in the paper (n = {len(matched)})",
        )
        ax1.scatter(
            mark["first_day_millions"],
            mark["effect"],
            s=58,
            color="#b4432f",
            edgecolor="white",
    ...
    ax1.set_ylabel("Excess fatalities on release day")
    ax1.set_title(
        "Every album on the US daily chart, release-day excess against dose",
        fontsize=12,
        loc="left",
    )
    ax1.legend(fontsize=9, loc="lower left", framealpha=0.9)

    binsrc = rest if cut is not None else fd
    binsrc = pd.concat([binsrc, matched]) if cut is not None else binsrc
    binsrc = binsrc.copy()
    binsrc["bin"] = pd.qcut(
        binsrc["first_day_millions"], n_bins, labels=False, duplicates="drop"
    )
    b = (
        binsrc.groupby("bin")
        .agg(
            dose=("first_day_millions", "median"),
            eff=("effect", "mean"),
            se=("effect", lambda x: x.std(ddof=1) / np.sqrt(len(x))),
            n=("effect", "size"),
        )
        .reset_index()
    )

    ax2.axhline(0, color="0.55", lw=1, zorder=1)
    ax2.errorbar(
        b["dose"],
        b["eff"],
        yerr=1.96 * b["se"],
    ...
    if len(mark):
        _m = mark["effect"].mean()
        _se = mark["effect"].std(ddof=1) / np.sqrt(len(mark))
        ax2.errorbar(
            [mark["first_day_millions"].median()],
            [_m],
            yerr=[1.96 * _se],
            fmt="o",
            color="#b4432f",
            ...
            markersize=8,
            zorder=5,
            label=f"the paper's {len(mark)}, pooled",
        )
        ax2.legend(fontsize=9, loc="upper left", framealpha=0.9)
    ax2.set_xscale("log")
    ax2.set_xlabel("Measured first-day US streams, millions (log scale)")
    ax2.set_ylabel("Mean excess fatalities")
    ax2.set_title(
        f"The same albums minus the paper's, in {len(b)} equal-count bins, "
        f"with 95% intervals",
        fontsize=11,
        loc="left",
    )
    ax2.text(
        0.985,
        0.04,
        f"across all {summary['n']} albums, including the paper's: "
        f"r = {summary['pearson']:+.3f}\n"
        f"(detectable above {summary['r_min_detectable']:.3f}), slope "
        f"{summary['slope_per_million']:+.3f} deaths per million",
