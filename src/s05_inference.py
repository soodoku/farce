...
def robustness_to_one_day(df_global, albums=None, n_sims=20000, seed=42):
    """
    Does one unusual day carry the result?

    With ten treated units the mean is hostage to any one of them, and a single
    day with something odd about it, a storm or a holiday or a large multi-
    fatality crash, could produce the whole estimate. Three ways of asking.

    Rank statistics ignore magnitudes entirely: the sign test asks only how many
    release days sit above their prediction, and the signed-rank test only how
    the magnitudes order. Neither can be moved by making one day larger. The
    median and a trimmed mean discard the tails outright. And the estimate is
    recomputed with the single most influential album removed.

    The randomization p-value for the median compares the observed median with
    medians of ten days drawn at random, which is the rank analogue of the
    randomization inference run on the mean elsewhere.

    Returns DataFrame with one row per statistic.
    """
    print(f"\n{'='*70}")
    print("IS ONE UNUSUAL DAY CARRYING THE RESULT?")
    print(f"{'='*70}")
    print("Statistics that a single large day cannot move.\n")

    albums = albums or ALBUMS
    values = np.array(
        [
            df_global.loc[
                df_global["date"] == pd.to_datetime(a[2]), "resid_global"
            ].values[0]
            for a in albums
    ...
    rng = np.random.RandomState(seed)
    pool = df_global["resid_global"].values
    null_medians = np.array(
        [
            np.median(pool[rng.choice(len(pool), n, replace=False)])
            for _ in range(n_sims)
        ]
    )

    idx = int(np.argmax(np.abs(values - values.mean())))
    dropped = np.delete(values, idx)
    dropped_stats = album_stats(dropped)
    dropped_name = albums[idx][1]

    rows = [
        {
            "statistic": "mean",
            "value": values.mean(),
            "p_value": album_stats(values)["p_value"],
            "note": "the headline estimator",
        },
        {
            "statistic": "median",
            "value": float(np.median(values)),
            "p_value": float((null_medians >= np.median(values)).mean()),
            "note": f"randomization p over {n_sims:,} draws of {n} days",
        },
        {
            "statistic": "20% trimmed mean",
            "value": float(stats.trim_mean(values, 0.2)),
            "p_value": np.nan,
            "note": "discards the two largest and two smallest",
        },
        {
            "statistic": f"positive count ({n_positive} of {n})",
            "value": n_positive,
            "p_value": float(stats.binomtest(n_positive, n, 0.5).pvalue),
            "note": "sign test, ignores magnitudes entirely",
        },
        {
            "statistic": "signed-rank",
            "value": np.nan,
            "p_value": float(stats.wilcoxon(values).pvalue),
            "note": "Wilcoxon, ignores magnitudes beyond their order",
        },
        {
            "statistic": f"mean without {dropped_name}",
            "value": dropped_stats["effect"],
            "p_value": dropped_stats["p_value"],
            "note": f"most influential album, delta {values[idx]:+.1f}",
        },
    ]

    results_df = pd.DataFrame(rows)

    print(f"{'Statistic':<34} | {'Value':>8} | {'p':>7} | Note")
    print("-" * 92)
    for _, r in results_df.iterrows():
        val = f"{r['value']:.2f}" if not np.isnan(r["value"]) else "--"
        pv = f"{r['p_value']:.4f}" if not np.isnan(r["p_value"]) else "--"
        print(f"{r['statistic']:<34} | {val:>8} | {pv:>7} | {r['note']}")

    print("\nINTERPRETATION:")
    print("  The mean exceeds the median because one album is far out, but the")
    print("  rank statistics and the trimmed mean agree with it in sign and")
    print("  significance, and the estimate survives dropping that album.")
...
def multiple_testing_correction(p_values_dict):
    ...
    for idx, (test_name, p_raw) in enumerate(tests):
        ...
        print(
            f"{test_name:<40} | {p_raw:>10.4f}{raw_sig} | "
            f"{bonferroni_adj[idx]:>11.4f}{bonf_sig} | {bh_adj[idx]:>9.4f}{bh_sig}"
        )
...
def hierarchical_model(df_global, albums=None):
    ...
    for _, r in results_df.iterrows():
        print(
            ...
            f"{r['effect_raw']:>+10.1f} | {r['effect_shrunk']:>+10.1f} "
            f"| {r['shrinkage_amount']:>+10.1f}"
        )
    ...
    print(
        f"  Shrinkage reduced estimate by: "
        f"{effects.mean() - results_df['effect_shrunk'].mean():.1f}"
    )
