...
from src.utils import add_time_features, build_design_matrix, cluster_robust_se, ols_fit
...
def local_estimate(df, window=10, pre_only=False):
    ...
    for artist, album, date_str, dow in ALBUMS:
        ...
        if pre_only:
            mask = (df["date"] >= dt - pd.Timedelta(days=window)) & (df["date"] < dt)
        else:
            mask = (
...
def paper_regression_estimate(df, albums=None, window=10, sample_period=None):
    ...
    beta, se_ols, fitted, resid = ols_fit(
        X.values, y, return_se=True, ridge_lambda=1e-8
    )
